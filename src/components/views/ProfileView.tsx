        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              <UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#cbc3d7]" />
              <input
                type="text"
                required
                value={name}
                onChange={(e) => setName(e.target.value)}
                className="w-full bg-black/20 border border-white/10 rounded-2xl pl-11 pr-4 py-3 text-white font-medium text-sm focus:outline-none focus:border-[#d0bcff]"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase text-[#cbc3d7] mb-1">Currency Symbol</label>
              <select
                value={currency}
                onChange={(e) => setCurrency(e.target.value)}
                className="w-full bg-[#171f33] border border-white/10 rounded-2xl px-4 py-3 text-white font-bold text-sm focus:outline-none focus:border-[#d0bcff]"
              >
            ...
            <div>
              <label className="block text-xs font-bold uppercase text-[#cbc3d7] mb-1">Default Expense Category</label>
              <select
                value={defaultCategory}
                onChange={(e) => setDefaultCategory(e.target.value)}
                className="w-full bg-[#171f33] border border-white/10 rounded-2xl px-4 py-3 text-white font-semibold text-sm focus:outline-none focus:border-[#d0bcff]"
              >
